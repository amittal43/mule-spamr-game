...
public class PubTest {
	...
	public void testNoTimeRemaining() {
		...
		// if there is no time left, the gambling winnings is equal to the round's gambling bonus
		for(int i=0; i<11; i++){
	...
	/* Since there is a random element involved in calculating the winnings,
	 * we must test that they fall within a specific range. We will test 100
	 * cases per each time remaining tested, in order to make sure the random
	 * */
	...
	public void test10secRange(){
		...
		for(int i=0; i<100; i++){
			assertTrue(pub.calcGamble(round, timeleft) >= round.getGamblingBonus());
			assertTrue(pub.calcGamble(round, timeleft) < round.getGamblingBonus() + 44); // 44 extra should be ceiling
	...
	public void test20secRange(){
		...
		for(int i=0; i<100; i++){
			assertTrue(pub.calcGamble(round, timeleft) >= round.getGamblingBonus());
			assertTrue(pub.calcGamble(round, timeleft) < round.getGamblingBonus() + 88); // 88 extra should be ceiling
	...
	public void test30secRange(){
		...
		for(int i=0; i<100; i++){
			assertTrue(pub.calcGamble(round, timeleft) >= round.getGamblingBonus());
			assertTrue(pub.calcGamble(round, timeleft) < round.getGamblingBonus() + 132); // 132 extra should be ceiling
	...
	public void test40secRange(){
		...
		for(int i=0; i<100; i++){
			assertTrue(pub.calcGamble(round, timeleft) >= round.getGamblingBonus());
			assertTrue(pub.calcGamble(round, timeleft) < round.getGamblingBonus() + 176); // 176 extra should be ceiling
	...
	public void test50secRange(){
		...
		for(int i=0; i<100; i++){
			assertTrue(pub.calcGamble(round, timeleft) >= round.getGamblingBonus());
			assertTrue(pub.calcGamble(round, timeleft) < round.getGamblingBonus() + 220); // 220 extra should be ceiling
